...
def create_model(units_mlp_x, units_mlp_s, units_lstm, units_mlp_c):
    ...
    out_final = layers.TimeDistributed(layers.Dense(t_max, activation='softmax', name='f_dense'),
...
if __name__ == '__main__':
    ...
    train_input, train_output, test_input, test_output = process_data(data, tkp.T_max_integrated)

    units = tkp.units_pi_int
    ...
    net.fit(x=train_input, y=train_output, epochs=15, batch_size=50)

    print("========= Evaluate =========")
    net.evaluate(test_input, test_output)

    net.save(tkp.net_path_pi_int)
